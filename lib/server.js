...
WalletService.prototype._canCreateAddress = function(ignoreMaxGap, cb) {
  ...
  self.storage.fetchAddresses(self.walletId, function(err, addresses) {
    ...
    self._getLatestActiveAddressIndex(latestAddresses, function(err, index) {
      return cb(err, index >= 0);
    });
  });
};
...
WalletService.prototype._getLatestActiveAddressIndex = function(addresses, cb) {
  var self = this;
  var bc = self._getBlockchainExplorer(addresses[0].network);
  ...
  async.whilst(function() {
    ...
    activityFound = !!address.hasActivity;
    if (address.hasActivity) return next();

    bc.getAddressActivity(address, function(err, res) {
      if (err) return next(err);
      activityFound = !!res;
      return next();
    });
    ...
    if (err) return cb(err);
    if (!activityFound) return cb(null, -1);
 ...
 * @returns {Address} address
 */
 WalletService.prototype.getNextChangeAddress = function(cb) {
   var self = this;
   ...
   self._runLocked(cb, function(cb) {
     self.getWallet({}, function(err, wallet) {
       if (err) return cb(err);
       if (!wallet.isComplete()) return cb(Errors.WALLET_NOT_COMPLETE);
       return self._getChangeAddress(wallet, function(err, address) {
         return self.storage.storeAddress(address, function(err) {
 ...
 WalletService.prototype._getUnusedChangeAddress = function(cb) {
   var self = this;

   self.storage.fetchAddresses(self.walletId, function(err, addresses) {
     if (err) return cb(err);
     var latestAddresses = _.takeRight(_.select(addresses, {
       isChange: true
     }), Defaults.MAX_MAIN_ADDRESS_GAP);

     if (latestAddresses.length === 0) return cb(err, null);

     self._getLatestActiveAddressIndex(latestAddresses, function(err, index) {
       if (index < latestAddresses.length - 1) return cb(err, latestAddresses[index + 1]);
       return cb(err, null);
     });
   });
 };

 WalletService.prototype._getChangeAddress = function(wallet, cb) {
   var self = this;
   ...
   if (wallet.singleAddress) {
     ...
     self._getUnusedChangeAddress(function(err, address) {
       if (err) return cb(err);
       if (address) return cb(null, address);
